...
### 3. 技術面について  
まず、5つの特徴量を与えたルールベースで学習させた1の経路決定AIを用いて「0:待機, 1:後退, 2:前進, 3:別ルートへ前進」の4種類の行動を出力し、そのあと、リアルタイムのプレイヤー行動から、
「左から攻める(ATTACK_LEFT)」「右から攻める(ATTACK_RIGHT)」「猛攻を仕掛ける(AGGRESSIVE)」「様子を見る(CAUTIOUS)」という行動方針を出力し、1の経路決定AIにバイアスをかけることで、
機械学習を行った。
